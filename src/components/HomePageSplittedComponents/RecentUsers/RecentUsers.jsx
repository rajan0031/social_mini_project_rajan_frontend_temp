    ...
    return (
        <div className="w-full p-6 bg-white rounded-lg shadow-xl h-full border border-gray-300">
            <h2 className="text-3xl font-bold mb-4 flex items-center text-purple-600">
                <FaCoffee className="inline-block mr-2 text-purple-500" /> Recent Users
            </h2>
            {/* Dummy recent users */}
            {["Recent User1", "Recent User2", "Recent User3", "Recent User4", "Recent User5"].map((user, index) => (
                <div key={index} className="flex items-center mb-3 p-2 border rounded hover:bg-purple-100 cursor-pointer transition duration-200 ease-in-out">
                    <FaUserCircle size={40} className="mr-2 text-purple-600" />
                    <span className="text-lg font-medium text-gray-800">{user}</span>
                </div>
            ))}

            <h2 className="text-3xl font-bold mt-6 mb-2 flex items-center text-purple-600">
                <FaRocket className="inline-block mr-2 text-purple-500" /> Join Us
            </h2>
